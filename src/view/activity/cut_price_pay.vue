    ...
    <div class="payDiv">
                ...
                <div class="divDiv">
                    ...
                    <div class="divFr">
                        <router-link
                            ...
                            '&is_group='+this.$route.query.is_group+'&usePlatform='+this.$route.query.usePlatform+'&useMerchat='+this.$route.query.useMerchat+'&from=pay&type=merchat'">
                            <div
                ...
                <div class="divDiv">
                    ...
                    <div class="divFr">
                        <router-link
                            :to="'/pay/coupon?goods_id='+this.$route.query.goods_id+'&goods_number='+this.number+
                            '&spec_key='+this.$route.query.spec_key+'&order_class='+this.$route.query.order_class+'&group_id='+this.$route.query.group_id+
                            '&is_group='+this.$route.query.is_group+'&usePlatform='+this.$route.query.usePlatform+'&useMerchat='+this.$route.query.useMerchat+'&from=pay&type=platform'">
                            <div
        ...
        <div class="payBtnDiv">
            <div class="payBottom">
                <p>
                    <label>实付款:</label>
                    <b v-if="surePrice">¥{{orderOnePrice}}</b>
                    <span>免运费</span>
                </p>
        ...
        methods: {
                    ...
                    .then(res => {
                        ...
                        if (self.merchatCouponList != '' && self.platformCouponList != '') {  // 如果商家优惠券和平台优惠券同时存在，先判断平台优惠券是否可以使用---规则：先扣除商家在扣除平台
                            ...
                            for (let i = 0; i < self.platformCouponList.length; i++) {
                                if (self.platformCouponList[i].condition_money <= self.orderPrice) {
                            ...
                            console.log(self.usePlatformCouponList.length)
                            if (self.usePlatformCouponList.length > 0 && this.$route.query.usePlatform != 'no') {
                                self.totalPrice = (self.orderPrice - self.platformCoupon.money).toFixed(2)
                            } else {
                                self.totalPrice = self.orderPrice
                            }


                            // 平台券减去成功后，去循环商家券，判断是否满足满减条件
                            ...
                            self.mechatCoupon = self.useMerchatCouponList.length > 0 ? self.useMerchatCouponList[self.mechatCouponIndex] : {}             // 默认商家券
            ...
            getOrder() {
                this.$post('buy/getBuyInfo', {
                    ...
                    .then(res => {
                        this.order = res;
                        console.log(JSON.stringify(this.order.pay_list))
                        this.payType = res.pay_list[0].pay_code
                        this.orderOnePrice = this.$route.query.order_class == '1' ? (res.goods_info.goods_price.price).toFixed(2) : (res.goods_info.goods_price.activity_price).toFixed(2)
                ...
                if (this.referer) {
                    if (JSON.stringify(this.order.default_address_info) != '{}') {
                        this.$post('buy/submit', {
                            ...
                            .then(res => {
                                if (res.code != 200) {
                                    Toast(res.msg + '前往砍价首页...');
                                    setTimeout(() => {
                                        self.$router.push(`/activity/bargain`)
                                    }, 2000)
                                } else {
