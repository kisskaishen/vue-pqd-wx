        ...
        <div class="chatList">
            ...
            <div class="chatMsg">
                ...
                <div class="chatMsgItem sendMsg" v-for="item,index in msgData">
                    <p class="chatTime" v-if="index<3">
                        {{item.msgModule.time | formatDate}}
                    </p>
                    <p class="chatTime" v-if="index>3&&item.msgModule.time-msgData[index-1].msgModule.time>60">
                        {{item.msgModule.time | formatDate}}
                    </p>
                    <!--商品信息-->
                    <div class="content goodsLink" v-if="item.goodsLink">
                        <div class="goodsPic">
                    ...
                    <div class="chatCon senderUser"
                         v-if="item.to=='store'+goodsInfo.merchant_info.store_id&&!item.goodsLink">
                        <div class="content">
                            <p v-if="item.data">{{item.data}}</p>
                            <p v-else-if="!item.data&&item.url">
                                <img :src="item.url" alt="">
                            </p>
                            <div v-else-if="item.data == ''&&item.msgModule.isGoods=='yes'">
                                <div class="goodsLink">
                                    <div class="goodsPic">
                                        <img :src="item.msgModule.goods.original_img" alt="">
                                    </div>
                                    <div class="goodsInfo">
                                        <p>{{item.msgModule.goods.goods_name}}</p>
                                        <p>
                                            <span>
                                                <span>￥<b>{{item.msgModule.goods.prom_price}}</b></span>
                                                <span>{{item.msgModule.goods.prom}}人团</span>
                                            </span>
                                        </p>
                                    </div>
                                </div>
                    ...
                    <!--接收者--商家-->
                    <div class="chatCon receiverUser" v-else-if="!item.goodsLink">
                        <div class="head_pic">
                            <img :src="item.msgModule.receiverUser.avatar" alt="">
                        </div>
                        <div class="content">
        ...
        methods: {
            ...
            getReply() {
                this.$post('Robotreply/getReply', {
                    ...
                    store_id: this.$route.query.store_id || this.goodsInfo.store_id
                })
                    .then(res => {
            ...
            setMsgModule() {
                let msgModule = {
                    ...
                    receiverUser: {
                        ...
                        userid: 'store' + this.goodsInfo.merchant_info.store_id,
            ...
            // 处理收到的消息
            dealMsg(msg) {
                console.log('这里是处理收到的信息')
                console.log(msg)
                if ((msg.from == 'store' + this.$route.query.store_id || msg.from == 'store' + this.goodsInfo.store_id) || (msg.to == 'store' + this.$route.query.store_id || msg.to == 'store' + this.goodsInfo.store_id)) {
                    this.msgData = this.msgData.concat(msg)
                }
            },
            // 发送图片
            sendImg() {
                ...
                var sendPrivateImg = function () {
                    ...
                    if (file.filetype.toLowerCase() in allowType) {
                        ...
                        let msgTep = {
                            url: file.url,          // 图片url
                            to: 'store' + self.$route.query.store_id || 'store' + self.goodsInfo.merchant_info.store_id,           // 发送给商家
                            from: self.user_id,              // 发送人user_id
            ...
            sendText(obj) {
                ...
                if (obj) {      // obj存在
                    if (obj.type == 'autoReply') {          // 点击自动回复
                        ...
                        msgModule.isGoods = 'yes'             // 把商品信息添加到消息体的对象
            ...
            goBottom() {
                this.$nextTick(() => {
                    ...
                    console.log(document.querySelector('.chatMsgItem').scrollHeight)
                    console.log(document.querySelector('.chatMsgItem').scrollTop)
                })
            },
